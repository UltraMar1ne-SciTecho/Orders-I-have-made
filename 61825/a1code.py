...
def print_stats(image):
    ...
    channels = 1 if image.ndim == 2 or (image.ndim == 3 and image.shape[2] == 1) else image.shape[2]
...
def resize(input_image, output_rows, output_cols):
    ...
    """
    out = skimage.transform.resize(input_image,
                                   (input_image.shape[0]*output_rows, input_image.shape[1]*output_cols),
...
def greyscale(input_image):
    ...
    out = skimage.color.rgb2gray(input_image)

    return out


def binary(gray_image, threshold):
    out = gray_image > threshold
    return out
...
def conv2D(image, kernel):
    ...
    out = scipy.ndimage.convolve(image, kernel, mode='constant', cval=0.0)
...
def corr(image, kernel):
    ...
    template_height, template_width = kernel.shape[0], kernel.shape[1]

    output_height = image_height - template_height + 1
    output_width = image_width - template_width + 1
    out = np.zeros((output_height, output_width))

    for y in range(output_height):
        for x in range(output_width):
            out[y, x] = np.sum(image[y:y + template_height, x:x + template_width] * kernel)
